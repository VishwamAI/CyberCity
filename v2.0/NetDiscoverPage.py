...
class NetDiscoverPage(Page):
    def __init__(self, back_label):
        ...
        try:
            logo_pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale('img/netdiscover.png', 300, 300, True)
        ...
        title = Gtk.Label()
        title.set_markup('<span size="xx-large">Netdiscover</span>')
        grid.attach(title, 1, 0, 1, 1)

        description = Gtk.Label()
        description.set_wrap(True)
        description.set_width_chars(40)
        description.set_markup('Netdiscover is a network address discovering tool.')
        ...
        link = Gtk.Label()
        link.set_markup('<a href="https://github.com/alexxy/netdiscover">Project Homepage</a>')
        link.set_use_markup(True)
        grid.attach(link, 1, 2, 1, 1)

        # Button for installation or uninstallation of Netdiscover
        ...
        # Button for about Netdiscover, now at the bottom
        about_button = Gtk.Button()
        about_button.set_child(Gtk.Image.new_from_file('img/netdiscover.png'))
        ...
        self.check_netdiscover_installed()

    def check_netdiscover_installed(self):
        try:
            output = subprocess.check_output(['which', 'netdiscover'], stderr=subprocess.STDOUT)
            if 'netdiscover' in output.decode():
                ...
                self._uninstall_handler_id = self.install_uninstall_button.connect("clicked", self.uninstall_netdiscover)
            else:
                ...
                self._install_handler_id = self.install_uninstall_button.connect("clicked", self.install_netdiscover)
    ...
    def install_netdiscover(self, button):
        subprocess.run(['sudo', 'apt-get', 'install', 'netdiscover', '-y'])
        self.check_netdiscover_installed()

    def uninstall_netdiscover(self, button):
        subprocess.run(['sudo', 'apt-get', 'remove', 'netdiscover', '-y'])
        self.check_netdiscover_installed()

    def share_snap(self, button):
        webbrowser.open('https://github.com/alexxy/netdiscover')

    def user_manual(self, button):
        webbrowser.open('https://manpages.ubuntu.com/manpages/focal/man8/netdiscover.8.html')

    def about(self, button):
        webbrowser.open('https://tools.kali.org/information-gathering/netdiscover')
