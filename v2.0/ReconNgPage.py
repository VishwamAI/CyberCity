...
class ReconNgPage(Page):
    def __init__(self, back_label):
        ...
        try:
            logo_pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale('img/recon-ng.png', 300, 300, True)
        ...
        title = Gtk.Label()
        title.set_markup('<span size="xx-large">Recon-ng</span>')
        grid.attach(title, 1, 0, 1, 1)

        description = Gtk.Label()
        description.set_wrap(True)
        description.set_width_chars(40)
        description.set_markup('Recon-ng is a full-featured Web Reconnaissance framework written in Python.')
        ...
        license = Gtk.Label()
        license.set_markup('License: BSD-3-Clause')
        grid.attach(license, 0, 2, 1, 1)

        link = Gtk.Label()
        link.set_markup('<a href="https://github.com/lanmaster53/recon-ng">Project Homepage</a>')
        link.set_use_markup(True)
        grid.attach(link, 1, 2, 1, 1)

        # Button for installation or uninstallation of Recon-ng
        ...
        # Button for about Recon-ng, now at the bottom
        about_button = Gtk.Button()
        about_button.set_child(Gtk.Image.new_from_file('img/recon-ng.jpg'))
        ...
        self.check_recon_ng_installed()
    def check_recon_ng_installed(self):
        try:
            output = subprocess.check_output(['which', 'recon-ng'], stderr=subprocess.STDOUT)
            if 'recon-ng' in output.decode():
                ...
                self._uninstall_handler_id = self.install_uninstall_button.connect("clicked", self.uninstall_recon_ng)
            else:
                ...
                self._install_handler_id = self.install_uninstall_button.connect("clicked", self.install_recon_ng)
    ...
    def install_recon_ng(self, button):
        subprocess.run(['sudo', 'apt-get', 'install', 'recon-ng', '-y'])
        self.check_recon_ng_installed()

    def uninstall_recon_ng(self, button):
        subprocess.run(['sudo', 'apt-get', 'remove', 'recon-ng', '-y'])
        self.check_recon_ng_installed()

    def share_snap(self, button):
        webbrowser.open('https://github.com/lanmaster53/recon-ng')

    def user_manual(self, button):
        webbrowser.open('https://github.com/lanmaster53/recon-ng/wiki')
