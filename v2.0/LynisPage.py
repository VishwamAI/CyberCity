...
class LynisPage(Page):
    def __init__(self, back_label):
        ...
        try:
            logo_pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale('img/lynis.png', 300, 300, True)
        ...
        title = Gtk.Label()
        title.set_markup('<span size="xx-large">Lynis</span>')
        grid.attach(title, 1, 0, 1, 1)

        description = Gtk.Label()
        description.set_wrap(True)
        description.set_width_chars(40)
        description.set_markup('Lynis is a security auditing tool for Unix and Linux systems.')
        ...
        license = Gtk.Label()
        license.set_markup('License: GNU General Public License v3.0')
        grid.attach(license, 0, 2, 1, 1)

        link = Gtk.Label()
        link.set_markup('<a href="https://cisofy.com/lynis/">Project Homepage</a>')
        link.set_use_markup(True)
        grid.attach(link, 1, 2, 1, 1)

        # Button for installation or uninstallation of Lynis
        ...
        # Button for about Lynis, now at the bottom
        about_button = Gtk.Button()
        about_button.set_child(Gtk.Image.new_from_file('img/lynis.png'))
        ...
        self.check_lynis_installed()

    def check_lynis_installed(self):
        try:
            output = subprocess.check_output(['which', 'lynis'], stderr=subprocess.STDOUT)
            if 'lynis' in output.decode():
                ...
                self._uninstall_handler_id = self.install_uninstall_button.connect("clicked", self.uninstall_lynis)
            else:
                ...
                self._install_handler_id = self.install_uninstall_button.connect("clicked", self.install_lynis)
    ...
    def install_lynis(self, button):
        subprocess.run(['sudo', 'apt-get', 'install', 'lynis', '-y'])
        self.check_lynis_installed()

    def uninstall_lynis(self, button):
        subprocess.run(['sudo', 'apt-get', 'remove', 'lynis', '-y'])
        self.check_lynis_installed()

    def share_snap(self, button):
        webbrowser.open('https://cisofy.com/lynis/')
