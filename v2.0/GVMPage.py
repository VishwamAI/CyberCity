...
class GVMPage(Page):
    def __init__(self, back_label):
        ...
        try:
            logo_pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale('img/gvm.png', 300, 300, True)
        ...
        title = Gtk.Label()
        title.set_markup('<span size="xx-large">Greenbone Vulnerability Manager (GVM)</span>')
        grid.attach(title, 1, 0, 1, 1)

        description = Gtk.Label()
        description.set_wrap(True)
        description.set_width_chars(40)
        description.set_markup('Greenbone Vulnerability Manager is a network vulnerability scanner with related tools.')
        ...
        link = Gtk.Label()
        link.set_markup('<a href="https://www.greenbone.net/en/">Project Homepage</a>')
        link.set_use_markup(True)
        grid.attach(link, 1, 2, 1, 1)

        # Button for installation or uninstallation of GVM
        ...
        # Button for about GVM, now at the bottom
        about_button = Gtk.Button()
        about_button.set_child(Gtk.Image.new_from_file('img/gvm.png'))
        ...
        self.check_gvm_installed()
    def check_gvm_installed(self):
        try:
            output = subprocess.check_output(['which', 'gvm'], stderr=subprocess.STDOUT)
            if 'gvm' in output.decode():
                ...
                self._uninstall_handler_id = self.install_uninstall_button.connect("clicked", self.uninstall_gvm)
            else:
                ...
                self._install_handler_id = self.install_uninstall_button.connect("clicked", self.install_gvm)
    ...
    def install_gvm(self, button):
        subprocess.run(['sudo', 'apt-get', 'install', 'gvm', '-y'])
        self.check_gvm_installed()

    def uninstall_gvm(self, button):
        subprocess.run(['sudo', 'apt-get', 'remove', 'gvm', '-y'])
        self.check_gvm_installed()

    def share_snap(self, button):
        webbrowser.open('https://www.greenbone.net/en/')

    def user_manual(self, button):
        webbrowser.open('https://docs.greenbone.net/')
